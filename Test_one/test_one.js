...
async function read_data_from_file(path, report_path) {
  ...
  Headerinfo["Requests"]=TxMessages.length
  head = "====";
  fs.appendFile(
    report_path,
    "\n" +
...
async function extract_slave_info(report_path) {
  ...
  if (slave_addresses.length !== 0) {
    for (index = 0; index < slave_addresses.length; index++) {
      ...
      device=parseInt(slave_addresses[index], 16)
      let devices = {
        
      };
      devices[device]=device_info
      device_details[device]=device_info
      // console.log(devices)
...
module.exports={
generate_report:async function(logpath) {
  var today = new Date();
  ...
  globalThis.device_details={}
globalThis.Headerinfo={};
  ...
  await read_data_from_file(logpath, report_filepath);
  ...
  Headerinfo["Unique_slaves"]=slave_addresses_in_int;
  Headerinfo["Responses"]=RxMessages.length
  Headerinfo["Device_details"]=device_details
  console.log(Headerinfo)
  fs.writeFile("Header_report.json",JSON.stringify(Headerinfo) , function(err, result) {
